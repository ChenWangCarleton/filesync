...
extern struct Queue *fn;//
extern struct QNode *n;//
//extern int filter;
static int watched_items;
extern int id;
char* bugfile=".goutputstream-";
...
void handle_event (queue_entry_t event)
{
  ...
  switch (event->inot_ev.mask & 
    ...
    case IN_CLOSE_WRITE:
		if(strstr(cur_event_filename, bugfile) != NULL) {
			//printf("%s ignored\n",cur_event_filename);
			break;
		}
	//	printf ("CLOSE_WRITE: %s \"%s\" on WD #%i\n",cur_event_file_or_dir, cur_event_filename, cur_event_wd);
		if(enQueue(fn,cur_event_filename,0)==0){	
			printf("file:%s is put in the queue  purpose: WRITE\n",cur_event_filename);
		}
		/*else{
			if(fn->capacity>0){
				n=deQueue(fn);
				printf("Dequeued item is %s, purpose is %s\n", n->fn,n->p);
			}
		}*/
		break;
    ...
    case IN_MOVED_FROM:
     ...
     // printf ("MOVED_FROM: %s \"%s\" on WD #%i. Cookie=%d\n",cur_event_file_or_dir, cur_event_filename, cur_event_wd, cur_event_cookie);
 		if(enQueue(fn,cur_event_filename,1)==0){	
			printf("file:%s is put in the queue  purpose: DELETE\n",cur_event_filename);
		}    
		break;
